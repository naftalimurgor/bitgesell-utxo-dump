...
import "github.com/naftalimurgor/bitgesell-utxo-dump/src/bitgesell/btcleveldb" // chainstate leveldb decoding functions
...
func main() {
    ...
    defaultfolder := fmt.Sprintf("%s/.BGL/chainstate/", os.Getenv("HOME")) // %s = string
    defaultfile := "blgutxodump.csv"
    ...
    if ! *nowarnings {
		cmd := exec.Command("BGL-cli", "getnetworkinfo")
		_, err := cmd.Output()
		if err == nil {
		    fmt.Println("Bitgesell is running. You should shut it down with `BGL-cli stop` first. We don't want to access the chainstate LevelDB while Bitcoin is running.")
		    fmt.Println("Note: If you do stop BGLd, make sure that it won't auto-restart (e.g. if it's running as a systemd service).")
    ...
    if ! *quiet {
		...
		if fieldsSelected["amount"] {
		    fmt.Printf("Total BTC:   %.8f\n", float64(totalAmount) / float64(100000000)) // convert  to BGL (float with 8 decimal places)
		}
